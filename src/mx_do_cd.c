...
static char *mx_cd_next(Process *p, int option_num) {
    char *pointer = NULL;
...
static void manage_env(char *dir, Prompt *shell,  cd_t cd_options, int *exit_code) {
...
static void print_error_cd(char *pointer) {
    mx_printerr("ush: cd: ");
    perror(pointer);
}

static int cd_check_path(char *pointer, cd_t cd_options) {
    int flag = 0;
...
static void mx_change_dir(char *pointer, cd_t cd_options, Prompt *shell, int *exit_code){
...
static char *mx_replace_sub(char *str, char *sub, char *replace) {
    char *result;
...
static char *mx_change_pwd(char **args, int option_num, Prompt *shell) {
    ...
    if (!mx_strstr(current, args[option_num + 1])) {
        ...
        new = mx_replace_sub(current, args[option_num + 1], args[option_num + 2]);
    }
    free(current);
    return new;
}

static void mx_fill_options_cd(int option_num, cd_t *cd_options, char **args) {
...
static int mx_count_args_cd(char **args, int option_num) {
    int args_num = 0;
...
int mx_cd(Prompt *shell, Process *p) {
    ...
    int args_num = mx_count_args_cd(p->argv, option_num);
    char *point = NULL;

    mx_fill_options_cd(option_num, &cd_options, p->argv);

    if (option_num >= 0 && args_num < 3) {
        if (args_num == 0) point = mx_cd_home();
        else if (args_num == 2) point = mx_change_pwd(p->argv, option_num, shell);
        else point = mx_cd_next(p, option_num);
