...
static char *mx_replace_substr(char *str,  char *sub, char *replace) {
...
static void mx_escape_seq(Process *proc, int i, echo_t echo_options) {
    ...
    for (int j = 0; sequenses[j] != NULL; j++) {
        if (strstr(proc->argv[i],sequenses[j])) {
            tmp = mx_replace_substr(proc->argv[i],sequenses[j], escape[j]);
            free(proc->argv[i]);
            proc->argv[i] = strdup(tmp);
            free(tmp);
        }
